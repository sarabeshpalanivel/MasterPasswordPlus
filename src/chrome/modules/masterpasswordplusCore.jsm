...
//									win.minimize();
			...
			{
				var p = Cc["@mozilla.org/preferences-service;1"].getService(Ci.nsIPrefService).getBranch("");
				this.lockPrefBackup = [];
				if (this.isTB)
				{
					if (this.pref.getBoolPref("hidenewmailalert"))
					{
						let b = p.getBoolPref("mail.biff.show_alert");
						function unload() p.setBoolPref("alert", a);
						this.lockPrefBackup.push([function unload() p.setBoolPref("alert", b), this.unload(unload)]);
						p.setBoolPref("alert", false);
					}

					if (this.pref.getBoolPref("hidenewmailballoon"))
					{
						let b = p.getBoolPref("mail.biff.show_balloon");
						function unload() p.setBoolPref("balloon", b);
						this.lockPrefBackup.push([unload, this.unload(unload)]);
						p.setBoolPref("balloon", false);
					}
				}
				if (Cc["@mozilla.org/preferences-service;1"].getService(Ci.nsIPrefService).getDefaultBranch("extensions.wmn.").getPrefType("showNotification") == Ci.nsIPrefBranch.PREF_BOOL)
				{
					if (this.pref.getBoolPref("hidenewmailalert"))
					{
						let b = p.getBoolPref("extensions.wmn.showNotification");
						function unload() p.setBoolPref("extensions.wmn.showNotification", c);
						this.lockPrefBackup.push([unload, this.unload(unload)]);
						p.setBoolPref("extensions.wmn.showNotification", false);
					}
				}
			}
			return;
...
//this.dump("\n" + k + "\n" + l + "\n-----");
	...
	onPrefChange: {
		...
		do: function(mapaPlus)
		{
			...
			mpc.prefShowChangesLog = mpc.pref.getBoolPref("showchangeslog");
